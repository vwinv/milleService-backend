...
let NotificationsService = class NotificationsService {
    ...
    static firebaseAdminMissingLogged = false;
    constructor(prisma) {
        this.prisma = prisma;
    }
    ...
    async sendPush(fcmToken, payload) {
        ...
        try {
            ...
            if (!admin) {
                if (process.env.NODE_ENV !== "test" &&
                    !NotificationsService_1.firebaseAdminMissingLogged) {
                    NotificationsService_1.firebaseAdminMissingLogged = true;
                    const detail = (0, firebase_admin_runtime_js_1.getFirebaseAdminLastLoadError)() ?? "inconnu";
                    this.logger.warn(`[FCM trace] Package firebase-admin introuvable (${detail}). Push FCM désactivé. ` +
